...
class ShellManager {
  static ShellInstance createInstance(
    ...
    if (Directory(instanceDir).existsSync()) {
      ...
      ProcessResult virtualEnvProcess = Process.runSync(
          config.defaultPythonPath, ["-m", "virtualenv", envDir]);

      if (virtualEnvProcess.exitCode == 0) {
        print(
            'virtualEnvProcess installed successfully exit code ${virtualEnvProcess.exitCode}.');
      } else {
        print('Error while installing virtualEnvProcess, exist code '
            '${virtualEnvProcess.exitCode}\nStdout: ${virtualEnvProcess.stdout}\n'
            'Stderr: ${virtualEnvProcess.stderr}');
      }
